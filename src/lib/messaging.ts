...
/**
 * The prospective-client persona shares the current client's seeded profile and
 * has no inbox of its own, so it is excluded to keep one person per profile ID.
 */
const MESSAGING_IDENTITIES = DEMO_IDENTITIES.filter(
  (identity) => identity.key !== "potential_client"
);

export const MESSAGING_PEOPLE: MessagingPerson[] = MESSAGING_IDENTITIES.map((identity) => ({
...
const personId = (role: UserRole) =>
  MESSAGING_IDENTITIES.find((identity) => identity.role === role)!.profileId;
