...
import { BILLING_ACTIVITIES, buildBillingCode } from "@/lib/billing-codes";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (editId) {
        ...
        if (row.billing_code) {
          const parts = String(row.billing_code).split("-");
          const maybeActivity = parts[parts.length - 1];
          if (maybeActivity && BILLING_ACTIVITIES.some((a) => a.code === maybeActivity)) {
            setActivityCode(maybeActivity);
          }
        }
